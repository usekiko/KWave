...
local function startProcessing()
    while (CurrentProgress ~= nil) do
        if CurrentProgress.length > 0 then
            ...
            ClearPedTasks(KW.PlayerData.ped)
...
local function Progressbar(message, length, Options)
    ...
    if CurrentProgress.animation then
        if CurrentProgress.animation.type == "anim" then
            KW.Streaming.RequestAnimDict(CurrentProgress.animation.dict, function()
                TaskPlayAnim(KW.PlayerData.ped, CurrentProgress.animation.dict, CurrentProgress.animation.lib, 1.0, 1.0, length, 1, 1.0, false, false, false)
                RemoveAnimDict(CurrentProgress.animation.dict)
            end)
        elseif CurrentProgress.animation.type == "Scenario" then
            TaskStartScenarioInPlace(KW.PlayerData.ped, CurrentProgress.animation.Scenario, 0, true)
        end
    end
    if CurrentProgress.FreezePlayer then
        FreezeEntityPosition(KW.PlayerData.ped, CurrentProgress.FreezePlayer)
    end
    SendNUIMessage({
        type = "Progressbar",
        length = length or 3000,
        message = message or "KW-Framework",
    })
...
KW.RegisterInput("cancelprog", "[ProgressBar] Cancel Progressbar", "keyboard", "BACK", CancelProgressbar)
