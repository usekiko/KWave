local Debug = KW.GetConfig().EnableDebug
...
RegisterNetEvent("KW:Notify", Notify)

if Debug then
    RegisterCommand("oldnotify", function()
        KW.ShowNotification('No Waypoint Set.', true, false, 140)
    end)

    RegisterCommand("notify", function()
        KW.ShowNotification("You Received <br>1x ball!", "success", 3000)
    end)

    RegisterCommand("notify1", function()
        KW.ShowNotification("Well ~g~Done~s~!", "success", 3000, "Achievement")
    end)

    RegisterCommand("notify2", function()
        KW.ShowNotification("Information Received", "info", 3000, "System Info")
    end)

    RegisterCommand("notify3", function()
        KW.ShowNotification("You Did something ~r~WRONG~s~!", "error", 3000, "Error")
    end)

    RegisterCommand("notify4", function()
        KW.ShowNotification("You Did something ~r~WRONG~s~!", "warning", 3000, "~y~Warning~s~")
    end)   
end