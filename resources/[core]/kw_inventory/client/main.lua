...
local function appendAccounts(elements)
    for i = 1, #(KW.PlayerData.accounts) do
        local account = KW.PlayerData.accounts[i]
        if account.money > 0 then
            local formattedMoney = TranslateCap("locale_currency", KW.Math.GroupDigits(account.money))
...
local function appendItems(elements)
    ...
    for i = 1, #KW.PlayerData.inventory do
        local item = KW.PlayerData.inventory[i]
...
local function appendLoadout(elements)
    ...
    for i = 1, #KW.PlayerData.loadout do
        local weapon = KW.PlayerData.loadout[i]
...
local function openQuantityDialog(title, maxCount)
    local p = promise:new()
    KW.UI.Menu.Open("dialog", KW.currentResourceName, "kw_inventory_quantity", {
        title = title
    }, function(data, menu)
        local qty = tonumber(data.value)
        if not qty or qty <= 0 or qty > maxCount then
            KW.ShowNotification(TranslateCap("amount_invalid"))
            p:resolve(nil)
        else
            menu.close()
            p:resolve(qty)
...
local function handleInventoryAction(action, selected, closestPlayer)
    local itemType, itemName, playerPed = selected.type, selected.value, KW.PlayerData.ped
    if action == "give" then
        local qty = openQuantityDialog(TranslateCap("amount"), selected.count)
        if not qty then return end
        TriggerServerEvent("kw:giveInventoryItem", GetPlayerServerId(closestPlayer), itemType, itemName, qty)
    elseif action == "remove" then
        local qty = openQuantityDialog(TranslateCap("amount"), selected.count)
        if not qty then return end
        TriggerServerEvent("kw:removeInventoryItem", itemType, itemName, qty)
    elseif action == "use" then
        TriggerServerEvent("kw:useItem", itemName)
    elseif action == "give_ammo" then
        ...
        if not qty then return end
        TriggerServerEvent("kw:giveInventoryItem", GetPlayerServerId(closestPlayer), "item_ammo", itemName, qty)
    end
end
...
local function showInventory()
    ...
    KW.UI.Menu.Open("default", KW.currentResourceName, "kw_inventory_main", {
        title = TranslateCap("inventory", totalWeight, KW.PlayerData.maxWeight),
        elements = elements,
    }, function(data, menu)
        local selected = data.current --[[@as InventoryElement]]
        if selected.unselectable then return end

        local closestPlayer, dist = KW.Game.GetClosestPlayer()
        local playerNearby = closestPlayer ~= -1 and dist <= 3.0

        KW.UI.Menu.Open("default", KW.currentResourceName, "kw_inventory_actions", {
...
KW.RegisterInput("showinv", TranslateCap("keymap_showinventory"), "keyboard", "F2", function()
...
local function refreshInventory()
    if not KW.UI.Menu.IsOpen("default", KW.currentResourceName, "kw_inventory_main") and
        not KW.UI.Menu.IsOpen("default", KW.currentResourceName, "kw_inventory_actions") and
        not KW.UI.Menu.IsOpen("dialog", KW.currentResourceName, "kw_inventory_quantity") then
        return
    end
    Citizen.Wait(0)
    KW.UI.Menu.CloseAll()
...
RegisterNetEvent("kw:addInventoryItem", refreshInventory)
RegisterNetEvent("kw:removeInventoryItem", refreshInventory)
