...
---@param length number
---@return string
function KW.GetRandomString(length)
    math.randomseed(GetGameTimer())

    return length > 0 and KW.GetRandomString(length - 1) .. Charset[math.random(1, #Charset)] or ""
end
...
---@return number, table
function KW.GetWeapon(weaponName)
...
---@return table
function KW.GetWeaponFromHash(weaponHash)
...
---@return table
function KW.GetWeaponList(byHash)
    return byHash and weaponsByHash or Config.Weapons
end

---@param weaponName string
---@return string
function KW.GetWeaponLabel(weaponName)
    weaponName = string.upper(weaponName)
...
---@return KWWeaponComponent?
function KW.GetWeaponComponent(weaponName, weaponComponent)
    weaponName = string.upper(weaponName)
    ...
    for _, component in ipairs(weapon.components) do
        ---@cast component KWWeaponComponent
...
---@param nb? number
---@return string
function KW.DumpTable(table, nb)
    if nb == nil then
        nb = 0
    end
...
---@return number
function KW.Round(value, numDecimalPlaces)
    return KW.Math.Round(value, numDecimalPlaces)
end

---@param value string
---@param ... any
---@return boolean, string?
function KW.ValidateType(value, ...)
    local types = { ... }
...
---@param ... any
---@return boolean
function KW.AssertType(...)
    local matches, errorMessage = KW.ValidateType(...)
...
---@param val unknown
function KW.IsFunctionReference(val)
...
function KW.Await(conditionFunc, errorMessage, timeoutMs)
    ...
    if not KW.IsFunctionReference(conditionFunc) then
    ...
    if errorMessage then
        KW.AssertType(errorMessage, "string", "errorMessage should be a string.")
    end
...
---@return boolean
function KW.IsValidLocaleString(str, allowDigits)
    if not KW.ValidateType(str, 'string') then
        return false
    end
