...
KW.currentResourceName = GetCurrentResourceName()
...
if not IsDuplicityVersion() then -- Only register this event for the client
    AddEventHandler("kw:setPlayerData", function(key, val, last)
        if GetInvokingResource() == "kw_core" then
    ...
    RegisterNetEvent("kw:playerLoaded", function(xPlayer)
        KW.PlayerData = xPlayer
        while not KW.PlayerData.ped or not DoesEntityExist(KW.PlayerData.ped) do Wait(0) end
    ...
    for i = 1, #external do
        ...
        local file = LoadResourceFile("kw_core", path)
        if file then
            local fn, err = load(file, ('@@kw_core/%s'):format(path))
    ...
    local function createStaticPlayer(src)
        return setmetatable({ src = src }, {
            __index = function(self, method)
                return function(...)
                    return exports.kw_core:RunStaticPlayerMethod(self.src, method, ...)
                end
            end
        })
    end
    ...
    ---@return StaticPlayer?
    function KW.Player(src)
        if type(src) ~= "number" then
            src = KW.GetPlayerIdFromIdentifier(src)
            if not src then return end
        elseif not KW.IsPlayerLoaded(src) then
            return
        end
    ...
    function KW.ExtendedPlayers(key, val)
        local playerIds = KW.GetExtendedPlayers(key, val, true)
...
if GetResourceState("ox_lib") == "missing" then
    ...
    local function getModuleInfo(modName)
        ...
        while true do
            ...
            if not src then
                return KW.currentResourceName, modName
            end

            resource = src:match('^@@([^/]+)/.+')

            if resource and not src:find('^@@kw_core/imports') then
