...
local function doesJobAndGradesExist(name, grades)
    if not KW.Jobs[name] then
       return false
    end

   for _, grade in ipairs(grades) do
       if not KW.DoesJobExist(name, grade.grade) then
           return false
       end
   end
...
function KW.CreateJob(name, label, grades, jobType)
    ...
    KW.Jobs[name] = generateNewJobTable(name, label, grades, jobType)

    notify("SUCCESS", currentResourceName, 'Job created successfully: `%s`', name)

    TriggerEvent('kw:jobCreated', name, KW.Jobs[name])
