...
---@param routingBucket? number
function KW.OneSync.GetPlayersInArea(source, maxDistance, ignore, routingBucket)
...
---@param ignore? table playerIds to ignore, where the key is playerId and value is true
---@param routingBucket? number
function KW.OneSync.GetClosestPlayer(source, maxDistance, ignore, routingBucket)
...
---@return number? netId
function KW.OneSync.SpawnVehicle(vehicleModel, coords, heading, vehicleProperties, cb, vehicleType)
    ...
    CreateThread(function()
        if not vehicleType then
            local xPlayer = KW.GetExtendedPlayers()[1]
            if xPlayer then
                vehicleType = KW.GetVehicleType(vehicleModel, xPlayer.source)
            end
        end
        ...
        local hasNetOwner = next(KW.OneSync.GetClosestPlayer(coords, 300, nil, 0) or {}) ~= nil
...
---@param cb? fun(netId: number)
---@return number? netId
function KW.OneSync.SpawnObject(model, coords, heading, cb)
...
---@param cb? fun(netId: number)
---@return number? netId
function KW.OneSync.SpawnPed(model, coords, heading, cb)
    if type(model) == "string" then
        model = joaat(model)
    end
...
---@param cb? fun(netId: number)
---@return number? netId
function KW.OneSync.SpawnPedInVehicle(model, vehicle, seat, cb)
    if type(model) == "string" then
        model = joaat(model)
    end
...
---@return table
function KW.OneSync.GetPedsInArea(coords, maxDistance, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return table
function KW.OneSync.GetObjectsInArea(coords, maxDistance, modelFilter)
...
---@return table
function KW.OneSync.GetVehiclesInArea(coords, maxDistance, modelFilter)
...
function KW.OneSync.GetClosestPed(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function KW.OneSync.GetClosestObject(coords, modelFilter)
...
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vector3 coords
function KW.OneSync.GetClosestVehicle(coords, modelFilter)
