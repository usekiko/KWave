...
function KW.TriggerClientEvent(eventName, playerIds, ...)
...
function KW.RegisterCommand(name, group, cb, allowConsole, suggestion)
    if type(name) == "table" then
        for _, v in ipairs(name) do
            KW.RegisterCommand(v, group, cb, allowConsole, suggestion)
        end
        return
    end
    ...
    RegisterCommand(name, function(playerId, args)
        ...
        if not command.allowConsole and playerId == 0 then
            ...
            local xPlayer, err = KW.Players[playerId], nil
            ...
            if command.suggestion then
                ...
                if not err and command.suggestion.arguments then
                    ...
                    for k, v in ipairs(command.suggestion.arguments) do
                        if v.type then
                            ...
                            elseif v.type == "player" or v.type == "playerId" then
                                ...
                                if targetPlayer then
                                    local xTargetPlayer = KW.GetPlayerFromId(targetPlayer)
                            ...
                            elseif v.type == "item" then
                                if KW.Items[args[k]] then
                                    newArgs[v.name] = args[k]
                                else
                                    err = TranslateCap("commanderror_invaliditem")
                                end
                            elseif v.type == "weapon" then
                                if KW.GetWeapon(args[k]) then
                        ...
                        if KW.IsFunctionReference(v.Validator?.validate) and not err then
    ...
    PostgreSQL.prepare(
        ...
        function(affectedRows)
            if affectedRows == 1 then
                ...
                TriggerEvent("kw:playerSaved", xPlayer.playerId, xPlayer)
            end
            if cb then
                cb()
            end
...
function Core.SavePlayers(cb)
    local xPlayers <const> = KW.Players
    ...
    PostgreSQL.prepare(
        ...
        function(results)
            ...
            print(("[^2INFO^7] Saved ^5%s^7 %s over ^5%s^7 ms"):format(#parameters,
                #parameters > 1 and "players" or "player", KW.Math.Round((os.time() - startTime) / 1000000, 2)))
        end
    )
end

KW.GetPlayers = GetPlayers
...
function KW.GetExtendedPlayers(key, val, minimal)
    if not key then
        if not minimal then
            return KW.Table.ToArray(KW.Players)
        end

        local xPlayers = {}
        local index = 1
        for src, _ in pairs(KW.Players) do
            xPlayers[index] = src
            index += 1
        end
...
---@param val? string|table
---@return number | table
function KW.GetNumPlayers(key, val)
    if not key then
        return #GetPlayers()
    end
    ...
    if type(val) == "table" then
        ...
        local filteredPlayers = KW.GetExtendedPlayers(key, val)
    ...
    return #KW.GetExtendedPlayers(key, val)
end

---@param source number
---@return xPlayer?
function KW.GetPlayerFromId(source)
    return KW.Players[tonumber(source)]
end

---@param identifier string
---@return xPlayer?
function KW.GetPlayerFromIdentifier(identifier)
    return Core.playersByIdentifier[identifier]
end

---@param identifier string
---@return number playerId
function KW.GetPlayerIdFromIdentifier(identifier)
...
function KW.IsPlayerLoaded(source)
    return KW.Players[source] ~= nil
end

---@param playerId number | string
---@return string, number
function KW.GetIdentifier(playerId)
    local fxDk = GetConvarInt("sv_fxdkMode", 0)
    if fxDk == 1 then
        return "KW-DEBUG-LICENCE", 0
    end
...
---@return string?
---@diagnostic disable-next-line: duplicate-set-field
function KW.GetVehicleType(model, player, cb)
    ...
    KW.TriggerClientCallback(player, "kw:GetVehicleType", function(vehicleType)
...
---@return nil
function KW.DiscordLog(name, title, color, message)
...
---@param fields table
---@return nil
function KW.DiscordLogFields(name, title, color, fields)
    for i = 1, #fields do
...
---@return nil
function KW.RefreshJobs()
    Core.JobsLoaded = false

    local Jobs = {}
    local jobs = PostgreSQL.query.await("SELECT * FROM jobs")
    ...
    local jobGrades = PostgreSQL.query.await("SELECT * FROM job_grades")
    ...
    for _, v in pairs(Jobs) do
        if KW.Table.SizeOf(v.grades) == 0 then
            Jobs[v.name] = nil
    ...
    if not Jobs then
        ...
        KW.Jobs["unemployed"] = { name = "unemployed", label = "Unemployed", type = "civ", whitelisted = false, grades = { ["0"] = { grade = 0, name = "unemployed", label = "Unemployed", salary = 200, skin_male = {}, skin_female = {} } } }
    else
        KW.Jobs = Jobs
    end
...
---@param cb function
---@return nil
function KW.RegisterUsableItem(item, cb)
...
---@param ... any
---@return nil
function KW.UseItem(source, item, ...)
    if KW.Items[item] then
        ...
        if itemCallback then
            ...
            if not success then
                ...
                print(('[^3WARNING^7] An error occured when using item ^5"%s"^7! This was not caused by KW.'):format(
                item))
            end
        end
    else
...
---@return nil
function KW.RegisterPlayerFunctionOverrides(index, overrides)
    Core.PlayerFunctionOverrides[index] = overrides
end

---@param index string
---@return nil
function KW.SetPlayerFunctionOverride(index)
...
---@return string?
---@diagnostic disable-next-line: duplicate-set-field
function KW.GetItemLabel(item)
    if KW.Items[item] then
        return KW.Items[item].label
    else
...
function KW.GetJobs(jobType)
    ...
    local filteredJobs = {}
    for jobName, jobObject in pairs(KW.Jobs) do
...
---@return table
function KW.GetItems()
    return KW.Items
end

---@return table
function KW.GetUsableItems()
    local Usables = {}
...
if not Config.CustomInventory then
    ...
    ---@return nil
    function KW.CreatePickup(itemType, name, count, label, playerId, components, tintIndex, coords)
        local pickupId = (Core.PickupId == 65635 and 0 or Core.PickupId + 1)
        local xPlayer = KW.Players[playerId]
        ...
        TriggerClientEvent("kw:createPickup", -1, pickupId, label, coords, itemType, name, components, tintIndex)
    ...
    local function refreshPlayerInventories()
        ...
        for i = 1, #xPlayers do
            ...
            for itemName, itemData in pairs(KW.Items) do
            ...
            TriggerClientEvent("kw:setInventory", xPlayer.source, xPlayer.inventory)
        end
    end

    ---@return number newItemCount
    function KW.RefreshItems()
        KW.Items = {}

        local items = PostgreSQL.query.await("SELECT * FROM items")
        local itemCount = #items
        for i = 1, itemCount do
            local item = items[i]
            KW.Items[item.name] = { label = item.label, weight = item.weight, rare = item.rare, canRemove = item
            .can_remove }
        end
    ...
    function KW.AddItems(items)
        ...
        if #toInsert > 0 then
            ...
            "INSERT INTO items (name, label, weight, rare, can_remove) VALUES (?, ?, ?, ?, ?) ON CONFLICT (name) DO NOTHING",
                toInsert)
...
---@param grade string
---@return boolean
function KW.DoesJobExist(job, grade)
    while not Core.JobsLoaded do
        Citizen.Wait(200)
    end

    return (KW.Jobs[job] and KW.Jobs[job].grades[tostring(grade)] ~= nil) or false
end
...
function Core.IsPlayerAdmin(playerSrc)
    ...
    local xPlayer = KW.GetPlayerFromId(playerSrc)
...
function Core.generateSSN(skipUniqueCheck)
    ...
    while true do
        ...
        local exists = PostgreSQL.scalar.await("SELECT 1 FROM users WHERE ssn = ? LIMIT 1", { candidate })
...
function KW.CreateExtendedVehicle(owner, plate, coords)
    return Core.vehicleClass.new(owner, plate, coords)
end

---@param plate string
---@return CExtendedVehicle?
function KW.GetExtendedVehicleFromPlate(plate)
