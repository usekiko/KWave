...
---@class KWInventoryWeapon:KWWeapon
...
---@field getAccount fun(account: string): KWAccount?            # Get account data by name.
---@field getAccounts fun(minimal?: boolean): KWAccount[]|table<string,number>  # Get all accounts, optionally minimal.
--- Inventory Functions
---@field getInventory fun(minimal?: boolean): KWInventoryItem[]|table<string,number>  # Get inventory, optionally minimal.
---@field getInventoryItem fun(itemName: string): KWInventoryItem? # Get a specific item from inventory.
...
---@field hasItem fun(item: string): KWInventoryItem|false, number? # Check if player has an item.
---@field getLoadout fun(minimal?: boolean): KWInventoryWeapon[]|table<string, {ammo:number, tintIndex?:number, components?:string[]}> # Get player's weapon loadout.
--- Job Functions
---@field getJob fun(): KWJob                                         # Get player's current job.
...
--- Properties
---@field accounts KWAccount[]     # Array of the player's accounts.
...
---@field inventory KWInventoryItem[] # Player's inventory items.
---@field job KWJob                # Player's current job.
---@field loadout KWInventoryWeapon[] # Player's current weapons.
...
---@param ssn string
---@param group string
---@param accounts KWAccount[]
---@param inventory table
---@param weight number
---@param job KWJob
---@param loadout KWInventoryWeapon[]
---@param name string
...
function CreateExtendedPlayer(playerId, identifier, ssn, group, accounts, inventory, weight, job, loadout, name, coords, metadata)
    ...
    stateBag:set("maxWeight", self.maxWeight, true)
    ...
    function self.set(k, v)
        ...
        self.triggerEvent('kw:updatePlayerData', 'variables', self.variables)
        Player(self.source).state:set("variables", self.variables, true)
    end
    ...
    function self.addInventoryItem(itemName, count)
        ...
        if item then
            count = KW.Math.Round(count)
            item.count = item.count + count
            self.weight = self.weight + (item.weight * count)

            TriggerEvent("kw:onAddInventoryItem", self.source, item.name, item.count)
            self.triggerEvent("kw:addInventoryItem", item.name, item.count)
        end
    end
        ...
        if item then
            ...
            if count > 0 then
                ...
                if newCount >= 0 then
                    ...
                    TriggerEvent("kw:onRemoveInventoryItem", self.source, item.name, item.count)
                    self.triggerEvent("kw:removeInventoryItem", item.name, item.count)
                end
            else
    ...
    function self.canCarryItem(itemName, count)
        if KW.Items[itemName] then
            local currentWeight, itemWeight = self.weight, KW.Items[itemName].weight
    ...
    function self.canSwapItem(firstItem, firstItemCount, testItem, testItemCount)
        ...
        if firstItemObject.count >= firstItemCount then
            local weightWithoutFirstItem = KW.Math.Round(self.weight - (firstItemObject.weight * firstItemCount))
            local weightWithTestItem = KW.Math.Round(weightWithoutFirstItem + (testItemObject.weight * testItemCount))
    ...
    function self.setMaxWeight(newWeight)
        ...
        self.triggerEvent("kw:setMaxWeight", self.maxWeight)
        Player(self.source).state:set("maxWeight", self.maxWeight, true)
    end
    ...
    function self.setJob(newJob, grade, onDuty)
        ...
        if not KW.DoesJobExist(newJob, grade) then
            return print(("[KW] [^3WARNING^7] Ignoring invalid ^5.setJob()^7 usage for ID: ^5%s^7, Job: ^5%s^7"):format(self.source, newJob))
        end
    ...
    function self.addWeapon(weaponName, ammo)
        if not self.hasWeapon(weaponName) then
            local weaponLabel <const> = KW.GetWeaponLabel(weaponName)
            ...
            self.triggerEvent("kw:addInventoryItem", weaponLabel, false, true)
            self.triggerEvent("kw:addLoadoutItem", weaponName, weaponLabel, ammo)
        end
    end
    ...
    function self.addWeaponComponent(weaponName, weaponComponent)
        ...
        if weapon then
            local component = KW.GetWeaponComponent(weaponName, weaponComponent)

            if component then
                if not self.hasWeaponComponent(weaponName, weaponComponent) then
                    self.loadout[loadoutNum].components[#self.loadout[loadoutNum].components + 1] = weaponComponent
                    local componentHash = KW.GetWeaponComponent(weaponName, weaponComponent).hash
                    GiveWeaponComponentToPed(GetPlayerPed(self.source), joaat(weaponName), componentHash)
                    self.triggerEvent("kw:addInventoryItem", component.label, false, true)
                end
            end
        end
    end
    ...
    function self.updateWeaponAmmo(weaponName, ammoCount)
        ...
        if weapon.ammo <= 0 then
            local _, weaponConfig = KW.GetWeapon(weaponName)
    ...
    function self.setWeaponTint(weaponName, weaponTintIndex)
        ...
        if weapon then
            local _, weaponObject <const> = KW.GetWeapon(weaponName)

            if weaponObject.tints and weaponObject.tints[weaponTintIndex] then
                self.loadout[loadoutNum].tintIndex = weaponTintIndex
                self.triggerEvent("kw:setWeaponTint", weaponName, weaponTintIndex)
                self.triggerEvent("kw:addInventoryItem", weaponObject.tints[weaponTintIndex], false, true)
            end
        end
    end
    ...
    function self.removeWeapon(weaponName)
        ...
        if weaponLabel then
            self.triggerEvent("kw:removeInventoryItem", weaponLabel, false, true)
            self.triggerEvent("kw:removeLoadoutItem", weaponName, weaponLabel)
        end
    end
    ...
    function self.removeWeaponComponent(weaponName, weaponComponent)
        ...
        if weapon then
            local component <const> = KW.GetWeaponComponent(weaponName, weaponComponent)
            ...
            if component then
                if self.hasWeaponComponent(weaponName, weaponComponent) then
                    ...
                    self.triggerEvent("kw:removeWeaponComponent", weaponName, weaponComponent)
                    self.triggerEvent("kw:removeInventoryItem", component.label, false, true)
                end
            end
        end
    end
    ...
    function self.showNotification(msg, notifyType, length, title, position)
        self.triggerEvent("kw:showNotification", msg, notifyType, length, title, position)
    end

    function self.showAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
        self.triggerEvent("kw:showAdvancedNotification", sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
    end

    function self.showHelpNotification(msg, thisFrame, beep, duration)
        self.triggerEvent("kw:showHelpNotification", msg, thisFrame, beep, duration)
    end
    ...
    function self.setMeta(index, value, subValue)
        ...
        self.triggerEvent('kw:updatePlayerData', 'metadata', self.metadata)
    ...
    function self.executeCommand(command)
        ...
        self.triggerEvent("kw:executeCommand", command)
    end
...
local function runStaticPlayerMethod(src, method, ...)
    local xPlayer = KW.Players[src]
    if not xPlayer then
        return
    end

    if not KW.IsFunctionReference(xPlayer[method]) then
