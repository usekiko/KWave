RegisterCommand("resetmigrations", function(src)
	...
	for version, _ in pairs(Core.Migrations or {}) do
		DeleteResourceKvp(("kw_migration:%s"):format(version))
	end
...
for esxVersion, migrations in pairs(Core.Migrations or {}) do
	...
	if KW.Table.SizeOf(migrations) > 0 then
		print(("^4[INFO]^7 Running migrations for KW version %s"):format(esxVersion))
		...
		SetResourceKvpInt(("kw_migration:%s"):format(esxVersion), 1)
		print(("^2[SUCCESS]^7 Successfully completed migrations for KW version %s"):format(esxVersion))
		migrationsRan += 1
	end
end

while restartRequired do
	print(("^4[INFO]^7 Ran migrations for %d KW version(s). ^1Server restart required!^7"):format(migrationsRan))
	Wait(500)
end
