import { pgsql_debug, pgsql_log_size, pgsql_slow_query_warning, pgsql_ui } from '../config';
import type { CFXCallback, CFXParameters } from '../types';
import { dbVersion } from '../database';

let loggerResource = '';
let loggerService = GetConvar('pgsql_logger_service', '');
...
export const logQuery = (
  ...
  if (
    executionTime >= pgsql_slow_query_warning ||
    (pgsql_debug && (!Array.isArray(pgsql_debug) || pgsql_debug.includes(invokingResource)))
  ) {
    console.log(
  ...
  logStorage[invokingResource].push({
    ...
    executionTime,
    date: Date.now(),
    slow: executionTime >= pgsql_slow_query_warning ? true : undefined,
  });
};
...
onNet(
  ...
  (data: {
    ...
    if (typeof data.resource !== 'string' || !IsPlayerAceAllowed(source as unknown as string, 'command.pgsql')) return;
