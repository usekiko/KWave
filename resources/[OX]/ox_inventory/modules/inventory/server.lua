...
function Inventory.Save(inv)
    ...
    if inv.player then
        return shared.framework ~= 'kw' and db.savePlayer(inv.owner, data)
...
function Inventory.Return(source)
	...
	local raw = PostgreSQL.scalar.await('SELECT data FROM ox_inventory WHERE name = ?', { inv.owner })

    if not raw then return end

	PostgreSQL.update.await('DELETE FROM ox_inventory WHERE name = ?', { inv.owner })

    -- oxpsql returns JSONB as native Lua table; guard against legacy TEXT rows
    local items = type(raw) == 'string' and json.decode(raw) or raw
...
local function prepareInventorySave(inv, buffer, time)
    ...
    if inv.player then
        if shared.framework == 'kw' then return end
