...
print('[^3DTF PvP^7] Weapon recoil system loaded')


-- ============================================
-- AIM DOWN SIGHTS - FIRST PERSON ON RMB
-- ============================================
...
CreateThread(function()
    while true do
        ...
        -- Check if player is aiming with a weapon (RMB / Right Mouse Button)
        local isAiming = IsControlPressed(0, 25) -- INPUT_AIM
        local hasWeapon = GetSelectedPedWeapon(ped) ~= -1569615261 -- not unarmed
        
        if hasWeapon and isAiming and not wasAiming then
            -- Started aiming - save current view mode and force first person
            previousViewMode = GetFollowPedCamViewMode()
            SetFollowPedCamViewMode(4) -- 4 = first person
            wasAiming = true
        elseif not isAiming and wasAiming then
            -- Stopped aiming - restore previous view mode
...
print('[^3DTF PvP^7] ADS First Person system loaded')
