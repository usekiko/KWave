...
-- NPC Death Detection (improved)
local function TrackNPCKills()
    if not Config.EnableNPCDeaths then return end
    
    CreateThread(function()
        local playerPed = PlayerPedId()
        local lastWeapon = nil
        local killedPedsBuffer = {} -- Buffer to track recently killed peds
        
        while true do
            Wait(50) -- Check every 50ms for faster detection
            
            -- Update player ped (in case of respawn)
            playerPed = PlayerPedId()
            
            -- Get current weapon
            local _, currentWeapon = GetCurrentPedWeapon(playerPed, true)
            if currentWeapon and currentWeapon ~= -1569615261 then
                lastWeapon = currentWeapon
            end
            
            -- Check all peds in area
            local peds = GetGamePool('CPed')
            local playerCoords = GetEntityCoords(playerPed)
            
            for _, ped in ipairs(peds) do
                if DoesEntityExist(ped) and not IsPedAPlayer(ped) then
                    -- Check if ped just died
                    if IsEntityDead(ped) then
                        local pedId = ped -- Use ped handle as ID
                        
                        -- Check if we already processed this death
                        if not lastKilledPeds[pedId] and not killedPedsBuffer[pedId] then
                            killedPedsBuffer[pedId] = true
                            ...
                            -- Multiple ways to detect if player killed this ped
                            local playerKilled = false
                            
                            -- Method 1: Direct source check
                            if killerPed == playerPed then
                                playerKilled = true
                            end
                            
                            -- Method 2: Check if player is in vehicle and caused damage
                            if not playerKilled and IsPedInAnyVehicle(playerPed, false) then
                                local playerVeh = GetVehiclePedIsIn(playerPed, false)
                                if playerVeh ~= 0 then
                                    -- Check if ped was hit by vehicle
                                    if causeOfDeath == -842959696 or causeOfDeath == 148160082 or causeOfDeath == -1200737721 then
                                        playerKilled = true
                                    end
                                end
                            end
                            
                            -- Method 3: Check if ped is close to player and recently damaged
                            if not playerKilled then
                                local pedCoords = GetEntityCoords(ped)
                                local dist = #(playerCoords - pedCoords)
                                if dist < 50.0 then
                                    -- Check if player has damaged this ped recently
                                    local health = GetEntityHealth(ped)
                                    if health <= 0 then
                                        -- Additional check: see if we're the nearest player
                                        playerKilled = true
                                    end
                                end
                            end
                            ...
                            if playerKilled then
                                ...
                                -- Get weapon used
                                local weaponHash = causeOfDeath or lastWeapon or -1569615261
                                local weaponInfo = GetWeaponInfo(weaponHash)
                                
                                -- Get NPC name
                                local npcName = GetNPCName(ped)
            ...
            -- Cleanup old entries (peds that no longer exist or expired)
            ...
            -- Clear buffer after a short delay to allow re-detection if needed
            if #peds == 0 then
                killedPedsBuffer = {}
            end
        end
...
-- Start NPC tracking if enabled
CreateThread(function()
    Wait(2000)
    if Config.EnableNPCDeaths then
        print('[^7DTF Death Messages^7] NPC kill tracking ENABLED')
        TrackNPCKills()
    else
