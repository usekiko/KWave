...
function Multicharacter:HideHud(hide)
    ...
    MumbleSetVolumeOverride(KW.PlayerId, 0.0)
    HideComponents(hide)
end
...
function Multicharacter:SetupCharacters()
    ...
    self.spawnCoords = Config.Spawn[KW.Math.Random(1,#Config.Spawn)]
    ...
    SetPlayerControl(KW.PlayerId, false, 0)
    self:SetupCamera()
    self:HideHud(true)
    ...
    SetTimeout(200, function()
        TriggerServerEvent("kw_multicharacter:SetupCharacters")
    end)
end
...
function Multicharacter:SpawnTempPed()
    ...
    KW.SpawnPlayer(skin, self.spawnCoords, function()
        DoScreenFadeIn(600)
        self.playerPed = PlayerPedId()
    end)
end
...
function Multicharacter:ChangeExistingPed()
    ...
    if spawnedCharacter and spawnedCharacter.model then
        local model = KW.Streaming.RequestModel(newCharacter.model)
        if model then
            SetPlayerModel(KW.playerId, newCharacter.model)
...
function Multicharacter:SetupUI(characters, slots)
    ...
    if not Character then
        ...
        KW.SpawnPlayer(Config.Default, self.spawnCoords, function()
            DoScreenFadeIn(400)
            self:AwaitFadeIn()
            ...
            TriggerServerEvent("kw_multicharacter:CharacterChosen", 1, true)
            TriggerEvent("kw_identity:showRegisterIdentity")
        end)
    else
        Menu:InitCharacter()
    end
...
function Multicharacter:LoadSkinCreator(skin)
    TriggerEvent("skinchanger:loadSkin", skin, function()
        ...
        TriggerEvent("kw_skin:openSaveableMenu", function()
...
function Multicharacter:SetDefaultSkin(playerData)
    ...
    model = KW.Streaming.RequestModel(model)

    if not model then
        return
    end

    SetPlayerModel(KW.playerId, model)
...
function Multicharacter:PlayerLoaded(playerData, isNew, skin)
    ...
    local esxSpawns = KW.GetConfig().DefaultSpawns
    ...
    self:DestoryCamera()
    KW.SpawnPlayer(skin, spawn, function()
        self:HideHud(false)
        SetPlayerControl(KW.playerId, true, 0)
