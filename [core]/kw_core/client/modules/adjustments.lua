...
function Adjustments:EnablePvP()
    if Config.EnablePVP then
        SetCanAttackFriendly(KW.PlayerData.ped, true, false)
...
local placeHolders = {
    server_name = function()
        return GetConvar("sv_projectName", "KW-Framework")
    end,
    ...
    player_name = function()
        return GetPlayerName(KW.playerId)
    end,
    player_rp_name = function()
        return KW.PlayerData.name or "John Doe"
    end,
    player_id = function()
        return KW.serverId
    end,
    player_street = function()
        if not KW.PlayerData.ped then return "Unknown" end
...
function Adjustments:WantedLevel()
    if not Config.EnableWantedLevel then
        ClearPlayerWantedLevel(KW.playerId)
        SetMaxWantedLevel(0)
    end
end
...
function Adjustments:DisableRadio()
    if Config.RemoveHudComponents[16] then
        AddEventHandler("kw:enteredVehicle", function(vehicle, plate, seat, displayName, netId)
