...
function KW.IsPlayerLoaded()
    return KW.PlayerLoaded
end

---@return table
function KW.GetPlayerData()
    return KW.PlayerData
end

---@param name string
---@param func function
---@return nil
function KW.SecureNetEvent(name, func)
    local invoker = GetInvokingResource()
    local invokingResource = invoker and invoker ~= 'unknown' and invoker or 'kw_core'
...
local addonResourcesState = {
    ['kw_progressbar'] = GetResourceState('kw_progressbar') ~= 'missing',
    ['kw_notify'] = GetResourceState('kw_notify') ~= 'missing',
    ['kw_textui'] = GetResourceState('kw_textui') ~= 'missing',
    ['kw_context'] = GetResourceState('kw_context') ~= 'missing',
}
...
function KW.DisableSpawnManager()
...
---@return table | number
function KW.SearchInventory(items, count)
    local item
    ...
    for i = 1, #KW.PlayerData.inventory do
        local e = KW.PlayerData.inventory[i]
        for ii = 1, #items do
...
---@return nil
function KW.SetPlayerData(key, val)
    local current = KW.PlayerData[key]
    KW.PlayerData[key] = val
    if key ~= "loadout" then
        if type(val) == "table" or val ~= current then
            TriggerEvent("kw:setPlayerData", key, val, current)
        end
    end
end
...
function Core.FreezePlayer(freeze)
    ...
    SetPlayerControl(KW.playerId, not freeze, 0)
...
---@return nil
function KW.SpawnPlayer(skin, coords, cb)
    local p = promise.new()
...
---@return nil
function KW.ShowNotification(message, notifyType, length, title, position)
	return IsResourceFound('kw_notify') and exports['kw_notify']:Notify(notifyType or "info", length or 5000, message, title, position)
end
...
---@return nil
function KW.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
...
---@return nil
function KW.ShowHelpNotification(msg, thisFrame, beep, duration)
...
---@return nil
function KW.ShowFloatingHelpNotification(msg, coords)
...
---@return nil
function KW.DrawMissionText(msg, time)
    ClearPrints()
...
function KW.HashString(str)
    return ('~INPUT_%s~'):format(('%x'):format(joaat(str) & 0x7fffffff + 2 ^ 31):upper())
end

function KW.OpenContext(...)
    return IsResourceFound('kw_context') and exports['kw_context']:Open(...)
end

function KW.PreviewContext(...)
    return IsResourceFound('kw_context') and exports['kw_context']:Preview(...)
end

function KW.CloseContext(...)
    return IsResourceFound('kw_context') and exports['kw_context']:Close(...)
end

function KW.RefreshContext(...)
    return IsResourceFound('kw_context') and exports['kw_context']:Refresh(...)
end
...
function KW.RegisterInput(command_name, label, input_group, key, on_press, on_release)
	local command = on_release and '+' .. command_name or command_name
    RegisterCommand(command, on_press, false)
    Core.Input[command_name] = KW.HashString(command)
    if on_release then
...
function KW.UI.Menu.RegisterType(menuType, open, close)
    KW.UI.Menu.RegisteredTypes[menuType] = {
        open = open,
        close = close,
    }
end
...
---@return KWMenu
function KW.UI.Menu.Open(menuType, namespace, name, data, submit, cancel, change, close)
    ---@class KWMenu
    local menu = {}
    ...
    menu.close = function()
        KW.UI.Menu.RegisteredTypes[menuType].close(namespace, name)

        for i = 1, #KW.UI.Menu.Opened, 1 do
            if KW.UI.Menu.Opened[i] then
                if KW.UI.Menu.Opened[i].type == menuType and KW.UI.Menu.Opened[i].namespace == namespace and KW.UI.Menu.Opened[i].name == name then
                    KW.UI.Menu.Opened[i] = nil
                end
            end
        end
    ...
    menu.refresh = function()
        KW.UI.Menu.RegisteredTypes[menuType].open(namespace, name, menu.data)
    end
    ...
    KW.UI.Menu.Opened[#KW.UI.Menu.Opened + 1] = menu
    KW.UI.Menu.RegisteredTypes[menuType].open(namespace, name, data)
...
---@return nil
function KW.UI.Menu.Close(menuType, namespace, name, cancel)
    for i = 1, #KW.UI.Menu.Opened, 1 do
        if KW.UI.Menu.Opened[i] then
            if KW.UI.Menu.Opened[i].type == menuType and KW.UI.Menu.Opened[i].namespace == namespace and KW.UI.Menu.Opened[i].name == name then
                if not cancel then
                    KW.UI.Menu.Opened[i].close()
                else
                    local menu = KW.UI.Menu.Opened[i]
                    KW.UI.Menu.RegisteredTypes[menu.type].close(menu.namespace, menu.name)
...
---@param cancel? boolean Should the close be classified as a cancel
---@return nil
function KW.UI.Menu.CloseAll(cancel)
    for i = 1, #KW.UI.Menu.Opened, 1 do
        if KW.UI.Menu.Opened[i] then
            if not cancel then
                KW.UI.Menu.Opened[i].close()
...
---@param name string
---@return KWMenu | nil
function KW.UI.Menu.GetOpened(menuType, namespace, name)
    for i = 1, #KW.UI.Menu.Opened, 1 do
        if KW.UI.Menu.Opened[i] then
            if KW.UI.Menu.Opened[i].type == menuType and KW.UI.Menu.Opened[i].namespace == namespace and KW.UI.Menu.Opened[i].name == name then
                return KW.UI.Menu.Opened[i]
            end
        end
    end
end
...
---@return KWMenu[]
function KW.UI.Menu.GetOpenedMenus()
    return KW.UI.Menu.Opened
end

KW.UI.Menu.IsOpen = KW.UI.Menu.GetOpened
...
---@return nil
function KW.UI.ShowInventoryItemNotification(add, item, count)
    SendNUIMessage({
...
function KW.Game.GetPedMugshot(ped, transparent)
...
function KW.Game.Teleport(entity, coords, cb)
...
---@return integer | nil
function KW.Game.SpawnObject(object, coords, cb, networked)
...
---@param cb? function The callback function
---@return nil
function KW.Game.SpawnLocalObject(object, coords, cb)
    KW.Game.SpawnObject(object, coords, cb, false)
end

---@param vehicle integer The vehicle to delete
---@return nil
function KW.Game.DeleteVehicle(vehicle)
...
---@return nil
function KW.Game.DeleteObject(object)
...
function KW.Game.SpawnVehicle(vehicleModel, coords, heading, cb, networked)
    if cb and not KW.IsFunctionReference(cb) then
    ...
    CreateThread(function()
        local modelHash = KW.Streaming.RequestModel(model)
...
---@param cb? function The callback function
---@return nil
function KW.Game.SpawnLocalVehicle(vehicle, coords, heading, cb)
    KW.Game.SpawnVehicle(vehicle, coords, heading, cb, false)
end

---@param vehicle integer The vehicle to check
---@return boolean
function KW.Game.IsVehicleEmpty(vehicle)
    return GetVehicleNumberOfPassengers(vehicle) == 0 and IsVehicleSeatFree(vehicle, -1)
end

---@return table
function KW.Game.GetObjects() -- Leave the function for compatibility
    return GetGamePool("CObject")
end

---@param onlyOtherPeds? boolean Whether to exlude the player ped
---@return table
function KW.Game.GetPeds(onlyOtherPeds)
    local pool = GetGamePool("CPed")

    if onlyOtherPeds then
        local myPed = KW.PlayerData.ped
        for i = 1, #pool do
...
---@return table
function KW.Game.GetVehicles() -- Leave the function for compatibility
...
---@return table
function KW.Game.GetPlayers(onlyOtherPlayers, returnKeyValue, returnPeds)
    local players = {}
    ...
    for i = 1, #active do
        ...
        if DoesEntityExist(ped) and ((onlyOtherPlayers and currentPlayer ~= KW.playerId) or not onlyOtherPlayers) then
...
function KW.Game.GetClosestObject(coords, modelFilter)
    return KW.Game.GetClosestEntity(KW.Game.GetObjects(), false, coords, modelFilter)
end

---@param coords? table | vector3 The coords to get the closest ped to
---@param modelFilter? table The model filter
---@return integer, integer
function KW.Game.GetClosestPed(coords, modelFilter)
    return KW.Game.GetClosestEntity(KW.Game.GetPeds(true), false, coords, modelFilter)
end

---@param coords? table | vector3 The coords to get the closest player to
---@return integer, integer
function KW.Game.GetClosestPlayer(coords)
    return KW.Game.GetClosestEntity(KW.Game.GetPlayers(true, true), true, coords, nil)
end

---@param coords? table | vector3 The coords to get the closest vehicle to
---@param modelFilter? table The model filter
---@return integer, integer
function KW.Game.GetClosestVehicle(coords, modelFilter)
    return KW.Game.GetClosestEntity(KW.Game.GetVehicles(), false, coords, modelFilter)
end
...
---@param maxDistance number The max distance to search within
---@return table
function KW.Game.GetPlayersInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(KW.Game.GetPlayers(true, true), true, coords, maxDistance)
end

---@param coords table | vector3 The coords to search from
---@param maxDistance number The max distance to search within
---@return table
function KW.Game.GetVehiclesInArea(coords, maxDistance)
    return EnumerateEntitiesWithinDistance(KW.Game.GetVehicles(), false, coords, maxDistance)
end

---@param coords table | vector3 The coords to search from
---@param maxDistance number The max distance to search within
---@return boolean
function KW.Game.IsSpawnPointClear(coords, maxDistance)
    return #KW.Game.GetVehiclesInArea(coords, maxDistance) == 0
end

---@param shape integer The shape to get the test result from
---@return boolean, table, table, integer, integer
function KW.Game.GetShapeTestResultSync(shape)
...
function KW.Game.RaycastScreen(depth, ...)
	local world, normal = GetWorldCoordFromScreenCoord(.5, .5)
	local origin = world + normal
	local target = world + normal * depth
	return target, KW.Game.GetShapeTestResultSync(StartShapeTestLosProbe(origin.x, origin.y, origin.z, target.x, target.y, target.z, ...))
end
...
---@param modelFilter? table The model filter
---@return integer, integer
function KW.Game.GetClosestEntity(entities, isPlayerEntities, coords, modelFilter)
...
function KW.Game.GetVehicleInDirection()
    local _, hit, coords, _, _, entity = KW.Game.RaycastScreen(5, 10, KW.PlayerData.ped)
...
---@return table | nil
function KW.Game.GetVehicleProperties(vehicle)
    ...
    return {
        ...
        plate = KW.Math.Trim(GetVehicleNumberPlateText(vehicle)),
        plateIndex = GetVehicleNumberPlateTextIndex(vehicle),

        bodyHealth = KW.Math.Round(GetVehicleBodyHealth(vehicle), 1),
        engineHealth = KW.Math.Round(GetVehicleEngineHealth(vehicle), 1),
        tankHealth = KW.Math.Round(GetVehiclePetrolTankHealth(vehicle), 1),

        fuelLevel = KW.Math.Round(GetVehicleFuelLevel(vehicle), 1),
        dirtLevel = KW.Math.Round(GetVehicleDirtLevel(vehicle), 1),
        color1 = colorPrimary,
...
---@return nil
function KW.Game.SetVehicleProperties(vehicle, props)
    if not DoesEntityExist(vehicle) then
        return
    end
    local colorPrimary, colorSecondary = GetVehicleColours(vehicle)
...
---@return nil
function KW.Game.Utils.DrawText3D(coords, text, size, font)
    local vector = type(coords) == "vector3" and coords or vec(coords.x, coords.y, coords.z)
...
---@return table|nil
function KW.GetAccount(account)
    for i = 1, #KW.PlayerData.accounts, 1 do
        if KW.PlayerData.accounts[i].name == account then
            return KW.PlayerData.accounts[i]
        end
    end
    return nil
end
...
RegisterNetEvent('kw:showAdvancedNotification', KW.ShowAdvancedNotification)

RegisterNetEvent('kw:showHelpNotification', KW.ShowHelpNotification)

AddEventHandler("onResourceStop", function(resourceName)
    for i = 1, #KW.UI.Menu.Opened, 1 do
        if KW.UI.Menu.Opened[i] then
            if KW.UI.Menu.Opened[i].resourceName == resourceName or KW.UI.Menu.Opened[i].namespace == resourceName then
                KW.UI.Menu.Opened[i].close()
                KW.UI.Menu.Opened[i] = nil
            end
        end
...
function KW.GetVehicleTypeClient(model)
...
KW.GetVehicleType = KW.GetVehicleTypeClient
