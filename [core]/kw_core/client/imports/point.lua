...
function Point:constructor(properties)
	...
	self.handle = KW.CreatePointInternal(properties.coords, properties.distance, properties.hidden, function()
...
function Point:delete()
	KW.RemovePointInternal(self.handle)
end
...
function Point:toggle(hidden)
	...
	self.hidden = hidden
	KW.HidePointInternal(self.handle, hidden)
end
