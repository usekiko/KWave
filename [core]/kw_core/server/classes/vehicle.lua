...
Core.vehicleClass = {
	...
	new = function(owner, plate, coords)
		...
		local vehicleProps = PostgreSQL.scalar.await("SELECT vehicle FROM owned_vehicles WHERE stored = true AND owner = ? AND plate = ? LIMIT 1", { owner, plate })
		...
		local netId = KW.OneSync.SpawnVehicle(vehicleProps.model, coords.xyz, coords.w, vehicleProps)
		if not netId then
			return
		end
		...
		PostgreSQL.update.await("UPDATE owned_vehicles SET stored = false WHERE owner = ? AND plate = ?", { owner, plate })

		local obj = table.clone(Core.vehicleClass)
		obj.plate = plate
		TriggerEvent("kw:createdExtendedVehicle", obj)
	...
	setPlate = function(self, newPlate)
		...
		local vehicleData = Core.vehicles[self.plate]
		local affectedRows = PostgreSQL.update.await("UPDATE owned_vehicles SET plate = ? WHERE plate = ? AND owner = ?", { newPlate, vehicleData.plate, vehicleData.owner })
		...
		TriggerEvent("kw:changedExtendedVehiclePlate", vehicleData.plate, oldPlate)
		Wait(0)
	...
	setOwner = function(self, newOwner)
		...
		local affectedRows = PostgreSQL.update.await("UPDATE owned_vehicles SET owner = ? WHERE owner = ? AND plate = ?", { newOwner, vehicleData.owner, vehicleData.plate })
		if affectedRows <= 0 then
			self:delete()
			return false
		end
	...
	delete = function(self, garageName, isImpound)
		...
		PostgreSQL.update.await(query, queryParams)
		TriggerEvent("kw:deletedExtendedVehicle", self)
