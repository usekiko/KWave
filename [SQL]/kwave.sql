...
CREATE INDEX idx_multicharacter_slots_slots ON multicharacter_slots (slots);
...
CREATE TABLE jobs (
  ...
  whitelisted SMALLINT NOT NULL DEFAULT 0
);
...
ALTER TABLE addon_account_data
  ADD CONSTRAINT index_addon_account_data_account_name_owner UNIQUE (account_name, owner);

CREATE INDEX index_addon_account_data_account_name ON addon_account_data (account_name);
...
--
CREATE INDEX index_addon_inventory_items_inventory_name_name ON addon_inventory_items (inventory_name, name);
CREATE INDEX index_addon_inventory_items_inventory_name_name_owner ON addon_inventory_items (inventory_name, name, owner);
CREATE INDEX index_addon_inventory_inventory_name ON addon_inventory_items (inventory_name);

--
-- Indexes for table billing
--
-- (PRIMARY KEY already defined inline via SERIAL)
...
ALTER TABLE datastore_data
  ADD CONSTRAINT index_datastore_data_name_owner UNIQUE (name, owner);

CREATE INDEX index_datastore_data_name ON datastore_data (name);
...
SELECT setval(pg_get_serial_sequence('addon_account_data', 'id'), COALESCE(MAX(id), 1)) FROM addon_account_data;
SELECT setval(pg_get_serial_sequence('datastore_data', 'id'), COALESCE(MAX(id), 1)) FROM datastore_data;
...
ALTER TABLE users ADD COLUMN pincode INTEGER NULL;
