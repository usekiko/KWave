...
SetTimeout(500, function()
	...
	for _, player in pairs(KW.Players) do
		if not Inventory(player.source) then
...
function server.hasLicense(inv, name)
	return PostgreSQL.scalar.await('SELECT 1 FROM user_licenses WHERE type = ? AND owner = ?', { name, inv.owner })
end
...
PostgreSQL.ready(function()
	PostgreSQL.update.await('INSERT INTO licenses (type, label) VALUES (?, ?) ON CONFLICT DO NOTHING', { 'weapon', 'Weapon License' })
end)
